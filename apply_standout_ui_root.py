...
# Configuration: Updating the ROOT directory now
...
def update_html_files():
    count = 0
    # We ignore the final-push folder to avoid confusion, focus on live ROOT
    for root, dirs, files in os.walk(TARGET_DIR):
        if "repairrange-final-push" in root:
            continue
        for file in files:
    ...
    print(f"Successfully updated {count} LIVE root files with the new standout UI theme.")
