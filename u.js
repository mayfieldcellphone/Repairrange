...
(async () => {
  const findAndClick = (fileName) => {
    const items = Array.from(document.querySelectorAll('[role="treeitem"]'));
    const target = items.find(item => item.textContent.trim().includes(fileName));
    if (target) {
      target.click();
      return true;
    }
  ...
  const setEditorValue = (newCode, fileName) => {
...
`;
  ...
  if (findAndClick(fileName)) {
    // Wait for editor to update (Monaco models are usually updated quickly)
    await new Promise(r => setTimeout(r, 2000));
    if (setEditorValue(newCode, fileName)) {
      return "App.tsx updated successfully";
    }
    return "App.tsx model not found after click";
  }
  return "App.tsx tree item not found";
})()
