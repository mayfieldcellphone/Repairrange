...
def call_gemini(prompt):
    ...
    for attempt in range(6):
        try:
            ...
            print(f"[gemini {e.code}] {e.read().decode()[:500]}"); raise
...
def run_site(site_key, dry):
    ...
    prompt = f"Write a blog for {cfg['domain']} about {topic}. Voice: {cfg['voice']}. Min 900 words. No prices. Australian spelling. Return ONLY JSON with keys: title, slug, meta_description, body_html."
    ...
    raise Exception("Generation failed after 3 attempts")
